...
import serviceAccountKey from "./quillvox-blog-firebase-adminsdk-4t3jb-1112276d21.json" assert { type: "json" };
...
const generateUploadURL = async () => {
    ...
    return await s3.getSignedUrlPromise('putObject', {
        Bucket: 'quillvox-blogging-website',
        Key: imageName,
        Expires: 1000,
...
server.post("/signup", (req, res) => {
    ...
    if(!emailRegex.test(email)){
        return res.status(403).json({ "error": "Email format invalid." })
    }
    if(!passwordRegex.test(password)){
        return res.status(403).json({ "error": "Password must be 6-20 characters long, with a numeric, 1 lowercase, and 1 uppercase character" })
    }
    ...
    .then((user) => {
        ...
        if(!user.google_auth){
            bcrypt.compare(password, user.personal_info.password, (err, result) => {
                if(err){
                    return res.status(403).json({ "error": "Error Occurred while login, try again." })
                }  
            ...
            return res.status(403).json({ "error": "Email is already used with Google Login. Sign in with google to continue." })
        }
...
server.post("/google-auth", async (req, res) => {
    ...
    .then(async (decodedUser) => {
        ...
        if(user) {
            if(!user.google_auth){
                return res.status(403).json({ "error": "This email was signed up without google. Please log in with email." })
            }
        }
        else{//signup
            let username = await generateUsername(email);
    ...
    .catch(err => {
        return res.status(500).json({ "error": "Failed tp authenticate, try other account" })
    })
...
server.post('/change-password', verifyJWT, (req, res) => {
    ...
    .then((user) => {
        ...
        if(user.google_auth){
            return res.status(403).json({ error: "You cannot change password as you logged in via Google." })
        } 
        ...
        bcrypt.compare(currentPassword, user.personal_info.password, (err, result) => {
            ...
            bcrypt.hash(newPassword, 10, (err, hashed_password) => {
                ...
                .catch(err => {
                    return res.status(500).json({ error: "Error occured while saving password." })
                })
        ...
        for(let i = 0; i < socialLinksArr.length; i++){
            ...
            if(social_links[socialLinksArr[i]].length){
                ...
                if(!hostname.includes(`${socialLinksArr[i]}.com`) && socialLinksArr[i] != 'website'){
                    return res.status(403).json({ error: `${socialLinksArr[i]} link is invalid. Enter a valid URL.` })
                }
...
server.post('/create-blog', verifyJWT, (req, res) => {
    ...
    if(!title.length) {
        return res.status(403).json({ error: "You must give your blog a title" })
    }
    ...
    if(!draft){
        ...
        if(!banner.length){
            return res.status(403).json({ error: "Give your blog a banner before publishing" })
        }
    
        if(!content.blocks.length) {
            return res.status(403).json({ error: "There must be some blog content to be published" })
        }
    
        if(!tags.length || tags.length > 7){
            return res.status(403).json({ error: "You can only add upto 7 tags" })
        }
    }
...
server.post("/add-comment", verifyJWT, (req, res) => {
    ...
    if(!comment.length){
        return res.status(403).json({ error: "Write some content for comment" });
    }
